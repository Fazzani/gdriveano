...
namespace ReleaseNotes
{
    internal class ReleaseNotesService : IReleaseNotesService
    {
        ...
        //TODO: Add support Webhooks for new ReleaseNotes
        //TODO: Insert new link for new added release into the summary page
        //TODO: Release note template from url
        //TODO: Add tag url (commit)
        //TODO: release notes strategies (tags or iterations) support
        ...
        public async Task UpdateOrCreateReleaseNotes(AppContext appContext, CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                ...
                appContext.WebApiTeam = await GetTeamByNameAsync(appContext.Connection,
                                                                 ...
                                                                 teamName: appContext.TeamName, cancellationToken).ConfigureAwait(false);
                ...
                foreach (var iter in selectedIterations)
                {
                    ...
                    if (notes.Count > 0)
                    {
                        var version = GetVersion2(appContext, iter);
        ...
        public string GetVersion2(AppContext appContext,
        ...
        public async ValueTask<string> GetVersion(AppContext appContext,
                                                  TeamSettingsIteration iteration,
                                                  string versionSuffix = "v2.",
        ...
        private async ValueTask<Wiql> GetQuery(WorkItemTrackingHttpClient witClient, AppContext appContext, TeamSettingsIteration iter, CancellationToken cancellationToken)
        {
            ...
            //var query = Regex.Replace(q.Wiql, @"@currentIteration\([^\)]+\)", match => $"{match.Value} {appContext.IterationOffset}", RegexOptions.Multiline | RegexOptions.IgnoreCase);
