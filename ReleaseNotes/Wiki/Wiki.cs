...
namespace ReleaseNotes.Wiki
{
    ...
    {
        public static async Task<WikiPageResponse> GetOrCreateWikiPage(VssConnection connection, Guid projectId, string pagePath = "ReleaseNotes")
        {
            ...
            try
            {
                return await wikiClient.GetPageAsync(projectId, wiki.Id, pagePath).ConfigureAwait(false);
            }
            catch
            {
